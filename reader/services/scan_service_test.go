...
func TestScanService_ScanDirectories_CreatesSeriesAndBooks(t *testing.T) {
	...
	scanSer := NewScanService(f.DBService, cfgPkg, f.LibraryService, f.SeriesService, f.BookService, f.CoverService, mockScanner)

	err := scanSer.ScanDirectories(context.Background())
	require.NoError(t, err)

	// Verify library was created
	libs, err := f.LibraryService.List(context.Background())
	require.NoError(t, err)
	require.Len(t, libs, 1)
	require.Equal(t, "Test Library", libs[0].Name)

	// Verify series was created
	series, err := f.SeriesService.MaybeGetByName(context.Background(), "Test Series")
	require.NoError(t, err)
	require.NotNil(t, series)

	// Verify book was created
	seriesBooks, total, err := f.BookService.ListBySeries(context.Background(), series.ID, 0, 10)
	require.NoError(t, err)
...
func TestScanService_ScanDirectories_MultipleSeriesAreCreated(t *testing.T) {
	...
	seriesA, err := f.SeriesService.MaybeGetByName(context.Background(), "Series A")
	require.NoError(t, err)
	require.NotNil(t, seriesA)

	seriesB, err := f.SeriesService.MaybeGetByName(context.Background(), "Series B")
	require.NoError(t, err)
	require.NotNil(t, seriesB)

	// Verify each series has one book
	booksA, _, err := f.BookService.ListBySeries(context.Background(), seriesA.ID, 0, 10)
	require.NoError(t, err)
	require.Len(t, booksA, 1)

	booksB, _, err := f.BookService.ListBySeries(context.Background(), seriesB.ID, 0, 10)
	require.NoError(t, err)
...
func TestScanService_ScanDirectories_EpisodePageCounts(t *testing.T) {
	...
	series, err := f.SeriesService.MaybeGetByName(context.Background(), "Test Series")
	require.NoError(t, err)
	books, _, err := f.BookService.ListBySeries(context.Background(), series.ID, 0, 10)
	require.NoError(t, err)
...
func TestScanService_ScanDirectories_IdempotentOnSecondScan(t *testing.T) {
	...
	seriesA, err := f.SeriesService.MaybeGetByName(context.Background(), "Test Series")
	require.NoError(t, err)
	require.NotNil(t, seriesA)

	booksA, totalA, err := f.BookService.ListBySeries(context.Background(), seriesA.ID, 0, 10)
	require.NoError(t, err)
	require.Len(t, booksA, 1)

	bookA, err := f.BookService.GetByID(context.Background(), booksA[0].ID)
	require.NoError(t, err)
	...
	seriesB, err := f.SeriesService.MaybeGetByName(context.Background(), "Test Series")
	require.NoError(t, err)
	require.NotNil(t, seriesB)
	...
	booksB, totalB, err := f.BookService.ListBySeries(context.Background(), seriesB.ID, 0, 10)
	require.NoError(t, err)
	require.Len(t, booksB, 1)
	require.Equal(t, totalA, totalB)

	bookB, err := f.BookService.GetByID(context.Background(), booksB[0].ID)
	require.NoError(t, err)
